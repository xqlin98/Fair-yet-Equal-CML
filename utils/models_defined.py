...
class MLP_HFT(nn.Module):

    def __init__(self, input_dim=144, output_dim=3, device=None):
        super(MLP_HFT, self).__init__()
...
class MLP_FRAUD(nn.Module):

    def __init__(self, input_dim=261, output_dim=2, device=None):
        super(MLP_FRAUD, self).__init__()
...
# For electricity time series predication
class RNN_ELEC(nn.Module):

    def __init__(self, input_size=40, hidden_size=40, device=None):
        super(RNN_ELEC, self).__init__()
    ...
    def forward(self, x):
        ...
        # x = F.sigmoid(torch.mean(output, axis=1))
        # x = output[:,-1,:]
...
#     def __init__(self, in_channels=3, num_classes=9, device=None):
...
class MEDNET(nn.Module):
    def __init__(self, in_channels=3, n_kernels=16, out_dim=9, device=None):
        super(MEDNET, self).__init__()
